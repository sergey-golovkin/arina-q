...
BEGIN
    ...
    (
      V_MSG_ID,
      UTL_COMPRESS.LZ_COMPRESS(CLOB2BLOB(P_DATA_VALUE), 9),
      P_META_INFO
    );
...
AS
  V_TABLE STQ_TABLE_10 := STQ_TABLE_10();
BEGIN
    SELECT CAST(MULTISET(SELECT ROWIDTOCHAR(STAGES_LOG_ARCHIVE.ROWID) ROW_ID, EXT_ID, SYS_ID, REQUEST_DATE, STAGE_DATE, REQUEST_ID, BLOB2CLOB(UTL_COMPRESS.LZ_UNCOMPRESS(DATA_VALUE)) DATA_VALUE, META_INFO, STAGE, ITERATION FROM STAGES_LOG_ARCHIVE, STAGES_MSG WHERE STAGES_LOG_ARCHIVE.MSG_ID = STAGES_MSG.MSG_ID AND REQUEST_ID = P_REQUEST_ID AND FINAL = 1) AS STQ_TABLE_10) INTO V_TABLE FROM DUAL;
    RETURN V_TABLE;
    EXCEPTION
    WHEN OTHERS
    THEN
